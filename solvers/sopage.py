...
class Solver(BaseSolver):
    ...
    def set_objective(self, f_train, f_val, n_inner_samples, n_outer_samples,
        ...
        if self.framework == 'numba':
            ...
            def sopage(*args, **kwargs):
                return njit_sopage(njit_vr, *args, **kwargs)
            self.sopage = sopage
            ...
            def sopage(*args, **kwargs):
                return _sopage(variance_reduction, *args, **kwargs)
            self.sopage = sopage
    ...
    def run(self, callback):
        ...
        while callback():
            if self.framework == 'jax':
                inner_var, outer_var, v, memory, carry = self.sopage(
                    self.f_inner, self.f_outer,
                    inner_var, outer_var, v, memory,
                    max_iter=eval_freq, **carry
                )
            else:
                inner_var, outer_var, v = self.sopage(
                    self.f_inner, self.f_outer,
                    inner_var, outer_var, v, memory,
...
def _sopage(variance_reduction, inner_oracle, outer_oracle, inner_var, outer_var,
...
def sopage_jax(f_inner, f_outer, inner_var, outer_var, v, memory,
    ...
    def sopage_one_iter(carry, _):
        memory, carry = carry
        (inner_step_size, outer_step_size), carry['state_lr'] = update_lr(carry['state_lr'])

        # Get all gradient for the batch
        start_inner, id_inner, weight_inner, carry['state_inner_sampler'] = (inner_sampler(carry['state_inner_sampler']))
